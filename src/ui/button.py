...
class GlobalButton:
    ...
    def __init__(self, label: str, button_style_map:tuple) -> object:
        ...
        self.disabled = True
        self.toggle = "None"
        self.width = len(label) + len(self.toggle) + 1  # padding
    ...
    def update_button(self, y: int, x:int, disabled: bool, toggle: int):
        ...
        if toggle == 0:
            self.toggle = "None" 
        elif toggle < 4:  #got 4 static dsahboards in total
            self.toggle = "Some"
        else:
            self.toggle = " All" #padding manually

        self.width = len(self.label) + len(self.toggle) + 6  # padding. when rendering, extra characters are added
    ...
    def draw(self, win: curses.window):
        if self.disabled is False:
            ...
            if toggle == "None":
                ...
                attr_toggle = self.button_style_map[1] #red

            # Draw label centered
            win.addstr(y + 1, x + 1, f"| {self.label} ", attr_label)
            win.addstr(f"{self.toggle}", attr_toggle)
